%------------------------------------------------------------------------------
  %
  %  Filename       : test_custom_crc.m
  %  Author         : Huang Leilei
  %  Status         : draft
  %  Created        : 2026-03-03
  %  Description    : test the custom_crc
  %
%------------------------------------------------------------------------------
...
for idxTest = 1:100
    ...
    % print
    mkdir(['test_vector/', num2str(idxTest)]);
    fp_inp = fopen(['test_vector/', num2str(idxTest), '/CRC_dumpDatInp_S1I8F0.dat'], 'w');
    fp_out = fopen(['test_vector/', num2str(idxTest), '/CRC_dumpDatOut_S1I32F0.dat'], 'w');
    fp_cfg = fopen(['test_vector/', num2str(idxTest), '/dut_setting.vh'], 'w');
    fprintf(fp_cfg, "// CRC\n");
