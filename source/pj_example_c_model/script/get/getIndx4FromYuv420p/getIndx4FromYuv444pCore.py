...
#------------------------------------------------------------------------------
    #
    #  Filename       : getIndx4FromYuv444pCore.py
    #  Author         : Huang Leilei
    #  Status         : draft
    #  Created        : 2025-02-18
    #  Description    : generate index4 files from yuv444p files (core)
    #
#-------------------------------------------------------------------------------
...
#*** PARAMETER *****************************************************************
CSTR_USAGE = "[information from {:s}] usage: ./getIndx4FromYuv444pCore.py <input file>, <frame number>, <frame width>, <frame height> >> <output file>\n".format(sys.argv[0])
try:
...
for idxFrame in range(int(NUMB_FRAME)):
    ...
    for idxY in range(SIZE_FRAME_Y):
        for idxX in range(SIZE_FRAME_X):
            datOut[0] = datInp[0][idxY][idxX] // 64
