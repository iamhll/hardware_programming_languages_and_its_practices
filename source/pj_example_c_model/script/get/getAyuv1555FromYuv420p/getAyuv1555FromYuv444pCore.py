...
#------------------------------------------------------------------------------
    #
    #  Filename       : getAyuv1555FromYuv444pCore.py
    #  Author         : Huang Leilei
    #  Status         : draft
    #  Created        : 2025-02-18
    #  Description    : generate ayuv1555 files from yuv444p files (core)
    #
#-------------------------------------------------------------------------------
...
#*** PARAMETER *****************************************************************
CSTR_USAGE = "[information from {:s}] usage: ./getAyuv1555FromYuv444pCore.py <input file> <frame number> <frame width> <frame height> <output file>\n".format(sys.argv[0])
try:
...
for idxFrame in range(int(NUMB_FRAME)):
    ...
    for idxY in range(SIZE_FRAME_Y):
        for idxX in range(SIZE_FRAME_X):
            datAlpha = 1 if datInp[0][idxY][idxX] > 128 else 0
            datY5bit = (datInp[0][idxY][idxX] & 0b11111000) >> 3
            datU5bit = (datInp[1][idxY][idxX] & 0b11111000) >> 3
            datV5bit = (datInp[2][idxY][idxX] & 0b11111000) >> 3
            datOut[0] = (datAlpha << 15) | (datY5bit << 10) | (datU5bit << 5) | datV5bit
