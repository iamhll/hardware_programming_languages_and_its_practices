...
calc_cfg_t *cfg)
{
    ...
    cout << endl;
    cout << "[xor]" << endl;
    if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH   ) cout << "    xorFlgProc          enable flag for processing" << endl;
    if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH   ) cout << "    xorEnmLoad          enumerator for load source (<value> 0: pipeline 1: file 2: random)" << endl;
    if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH   ) cout << "    xorEnmDump          enumerator for data dump (<value> 0: none 1: inout 2: intermediate)" << endl;
    cout << endl;
    cout << "[example]" << endl;
...
const string *strKey, const string *strDat)
{
    ...
    else if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH    && (*strKey == "--xorFlgProc"                           ))    cfg->xorFlgProc            = datAsBool;
    else if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH    && (*strKey == "--xorEnmLoad"                           ))    cfg->xorEnmLoad            = datAsInt;
    else if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH    && (*strKey == "--xorEnmDump"                           ))    cfg->xorEnmDump            = datAsInt;
...
calc_cfg_t *cfg)
{
    ...
    if (cfg->xorEnmLoad < 0 || cfg->xorEnmLoad > 2) {
        cerr << "[error from c model] cfg->xorEnmLoad should be within [0,2]!" << endl;
        return 1;
    };
    if (cfg->xorEnmDump < 0 || cfg->xorEnmDump > 2) {
        cerr << "[error from c model] cfg->xorEnmDump should be within [0,2]!" << endl;
        return 1;
    };
...
calc_cfg_t *cfg)
{
    ...
    if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH   )    cout << "--xorFlgProc             " << cfg->xorFlgProc << endl;
    if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH   )    cout << "--xorEnmLoad             " << cfg->xorEnmLoad << endl;
    if (CALC_CFG_ENUM_LVL_RLS >= calc_enmLvlCfg_t::ROUGH   )    cout << "--xorEnmDump             " << cfg->xorEnmDump << endl;
