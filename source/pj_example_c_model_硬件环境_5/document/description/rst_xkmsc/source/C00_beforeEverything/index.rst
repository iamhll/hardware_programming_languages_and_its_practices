...
#. |  enable WSL function
   |  (**Hyper-V, Virtual Machine Platform or Windows Hypervisor Platform may also be needed.**)

   .. image:: WSL_step3.png
      :width: 800

#. |  install Ubuntu
   |  (**Please install version 22.04 or 20.04.**)
...
#. open Ubuntu
   ...
   |  (**The user name must start with a lowercase letter.**)
   |  (**The interface does not display anything while the password is being entered, but the input is actually recorded.**)
...
#. |  open VS code and connect it to WSL
   |  (This step will automatically install "WSL" extension)

   .. image:: VSCode_step2.png
      :width: 800

#. |  install "C/C++" extension in WSL
   |  (If not connected to WSL, click the bottom-left corner and choose "connected to WSL")
...
#. install other extensions
   ...
   *  Binary (optional)
   *  C/C++ (**must**)
   *  Chinese (Simplified) Language Pack for Visual Studio code (optional)
   *  Git History (optional)
   *  Jupyter (optional)
   *  Tcl (optional)
   *  Verilog HDL (optional)
   *  ...
...
#. download :download:`pj_example_c_model.tar <../../../../../../pj_example_c_model.tar>` and put it to your home directory
...
#. |  open it with VS code connected to WSL
   |  (If not connected to WSL, click the bottom-left corner and choose "connected to WSL")

   .. image:: VSCode_step5.png
      :width: 800

#. open file *source/top/msc.cpp* (optional)

   .. image:: VSCode_step6.png
      :width: 800

#. |  press (Fn+)F5 to run gdb
   |  (**It appears that the following log is generated only after the second execution.**)
   |  (It seems WSL1 does not support gdb.)
   |  (To fix it, you can update it to WSL2 with "wsl --update" in "Windows PowerShell" environment.)
